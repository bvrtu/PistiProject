                ...
                // Try to parse the input as an integer.
                    try {
                        ...
                        if (index < 0 || index >= player.length) {
                            ...
                            if (table[0] != null && (player[index].getValue() == table[0].getValue() || player[index].getValue() == 11)) {
                                ...
                                // Indicates the last player on the table to receive the cards.
                                    lastTake = "player";
                            } else {
                    ...
                    // Remove the chosen card from the computer's hand.
                        ...
                        if (table[0] != null && (cardToPlay.getValue() == table[0].getValue() || cardToPlay.getValue() == 11)) {   
                            ...
                            if (nonNullElements == 1 && cardToPlay.getValue() != 11) {
                                computerPoints += 10;
                            } 
                            ...
                            // Print the cards on the table.
                                System.out.println("--------------------------");
                                System.out.print("Computer played: " + cardToPlay);
                                System.out.println();
                                System.out.println();
                                System.out.print("Card on top of the table: ");
                                if (table[0] != null) {
                            ...
                            if (turn == 5 && (player.length == 0 && computer.length == 0)) {
                                // Check if there are any cards left on the table.
                                    if (table[0] != null) {
                                        // Transfer the remaining cards on the table to player2 array if the last player to collect the cards was the player.
                                            if (lastTake.equals("player")) {
                                                // Create a new array for player2 array with the same number of elements as the number of cards on the table.
                                                    Card[] newPlayer2 = new Card[player2.length + table.length];
                                                // Add all elements from the table to player2 array, starting at the end of the old array.
                                                    System.arraycopy(table, 0, newPlayer2, player2.length, table.length);
                                                // Add all elements from player2's old array to the new one.
                                                    System.arraycopy(player2, 0, newPlayer2, 0, player2.length);
                                                // Replace player2's old array with the new one.
                                                    player2 = newPlayer2;
                                            
                                            // Transfer the remaining cards on the table to computer2 array if the last player to collect the cards was the computer.
                                            } else if (lastTake.equals("computer")) {
                                                // Create a new array for computer2 array with the same number of elements as the number of cards on the table.
                                                    Card[] newComputer2 = new Card[computer2.length + table.length];
                                                // Add all elements from the table to computer2's new array, starting at the end of the old array.
                                                    System.arraycopy(table, 0, newComputer2, computer2.length, table.length);
                                                // Add all elements from computer2's old array to the new one.
                                                    System.arraycopy(computer2, 0, newComputer2, 0, computer2.length);
                                                // Replace computer2's old array with the new one.
                                                    computer2 = newComputer2;
                                            }
                                // Clear the table.
                                    table = new Card[52];
